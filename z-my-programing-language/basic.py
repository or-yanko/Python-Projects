...
class Error:
    ...
    def as_string(self):
        ...
        result += f'File {self.pos_start.fn}, line {self.pos_start.ln + 1}, pos {self.pos_start.col + 1}'
        result += '\n\n' + \
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                self.advance()
                return [], IllegalCharError(pos_start, self.pos, "'" + char + "'"), None
...
def run(fn, text):
    # Generate tokens
    lexer = Lexer(fn, text)
    tokens, error, explenation = lexer.make_tokens()
    if error:
        return None, error
    if explenation:
        return tokens, None
